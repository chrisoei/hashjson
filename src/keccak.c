...
{
	s->taillen = 0;
    memset( s->state, 0, 5*5*sizeof(tKeccakLane) );
}

void SHA3_256_Update( SHA3_256_CTX *s, const unsigned char *in, unsigned long long inlen )
{
	// If the size of an input file is greater than cko_multidigest_t.chunksize,
	// SHA3_256_Update will be called multiple times in cko_multidigest_update().
	// So the tail left by previous call of that function must be hashed with the first bytes of the new input.
	// The tail should be preserved between calls of SHA3_256_Update because:
	// 1) KeccakF() can take only an input of size cKeccakR_SizeInBytes / sizeof(tKeccakLane) (17 for SHA3-256);
	// 2) It's impossible to pad the input to the required size, because it will change the length of the original input;
	// 3) Only the tail of the input file could be padded.

	if( s->taillen != 0 )
	{
		unsigned char temp[cKeccakR_SizeInBytes];
		int shift = cKeccakR_SizeInBytes - s->taillen;

		if( inlen >= shift )
		{
			memcpy( temp, s->tail, s->taillen );
			memcpy( temp + s->taillen, in, shift );

			KeccakF( s->state, (const tKeccakLane *)temp, cKeccakR_SizeInBytes / sizeof(tKeccakLane) );
		...
		{
			memcpy( s->tail + s->taillen, in, inlen );
			s->taillen += inlen;
			return;
		}
	}
    ...
    {
        KeccakF( s->state, (const tKeccakLane*)in, cKeccakR_SizeInBytes / sizeof(tKeccakLane) );
    }

	// saving the tail
	memcpy( s->tail, in, (size_t)inlen );
	s->taillen = inlen;
}
...
void SHA3_256_Final( unsigned char * out, SHA3_256_CTX *s )
{
    //    padding
    s->tail[s->taillen++] = 1;
    memset( s->tail+s->taillen, 0, cKeccakR_SizeInBytes - (size_t)s->taillen );
    s->tail[cKeccakR_SizeInBytes-1] |= 0x80;
    KeccakF( s->state, (const tKeccakLane*)(s->tail), cKeccakR_SizeInBytes / sizeof(tKeccakLane) );

    memcpy( out, s->state, crypto_hash_BYTES );
}
