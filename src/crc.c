...
crcFastInit(crc* remainderptr) {
   *remainderptr=INITIAL_REMAINDER;
   crcInit();
}
...
void crcFastUpdate(crc* remainderptr,cko_u8*  message, int nBytes)
{
    ...
    for (byte = 0; byte < nBytes; ++byte)
    {
        data = REFLECT_DATA(message[byte]) ^ ((*remainderptr) >> (WIDTH - 8));
  		*remainderptr = crcTable[data] ^ ((*remainderptr) << 8);
    }
}   /* crcFast() */

crc crcFastFinal(crc *remainderptr) {
    /*
     * The final remainder is the CRC.
     */
    *remainderptr = (REFLECT_REMAINDER(*remainderptr) ^ FINAL_XOR_VALUE);
}
